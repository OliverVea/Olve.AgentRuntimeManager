...
TypeSpec contract ([`src/spec/main.tsp`](../spec/main.tsp)). It shows a read-only, live list
of the newest agent sessions, proving the client-gen → component → API loop end to end
(DESIGN §2).
...
```
src/frontend/
├─ index.html                 the page (mounts <session-list> + the log in / out bar)
...
│  └─ components/
│     └─ session-list.ts      <session-list> — newest sessions, live from the event stream
```
...
Every endpoint except `GET /api/auth-config` needs a bearer token, so the list shows a sign-in
prompt until you click **Log in** to run the OIDC flow (see below); the access token is then
attached as a Bearer token automatically.
...
`<session-list>` driving the real generated client over a scripted `fetch` and a scripted event
stream (render, live updates from `session.*` events, the signed-out prompt, the error envelope's
message, the 401 path).
...
- Calls don't throw: they return `{ data, error, response }`. `session-list.ts` turns a failure
