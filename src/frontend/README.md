...
from `../../artifacts/backend/api.json` (written by the backend's `dotnet build`). It is **committed** so the app
...
> runtime. Kiota `1.32.5` (see `../backend/.config/dotnet-tools.json`) pairs with the
> `1.0.0-preview.103` runtime pinned in `package.json` — a mismatched runtime changes
> serializer signatures and breaks the build. Run `cd ../backend && dotnet tool run kiota info -d ../../artifacts/backend/api.json
