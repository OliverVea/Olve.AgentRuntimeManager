...
- [ ] M9 — **Claude provider**, moved ahead of M5a (2026-09-26): its process model decides what gentle restart has to do. Integration decided after spikes (A10): the CLI's `stream-json` protocol directly, no ACP adapter or Agent SDK sidecar. First cut: a `claude` provider that runs the locked-down CLI per session (the ARM session id as Claude's session id, the model passed through, the final result as the summary, the raw output kept per session), tested LLM-free against a stub CLI replaying recorded output (TESTING L2). Then Claude Code on the VMs: the latest release verified and staged in the bundle by a `claude-code` pipeline step (beta and prod run the same version; rollback included), logged in with `CLAUDE_CODE_OAUTH_TOKEN` from a pipeline secret per environment; then provider health: a provider refusing agents (bad token, usage limit, outage) pauses, its sessions queue and retry (`GET /api/providers/health`, `arm provider health`, `provider.health` events; 2026-09-26). Steering, interrupt and approvals use the same channel and land with their milestones (M11, M8)
- [ ] M5a — **Gentle restart**: server exits, agents survive, new server re-attaches (event replay completes in M6). A supervisor per session, a separate internal executable in the release, owns the agent's stdio, logs its output and exit code to disk and lets a restarted ARM reconnect over a socket; if the supervisor is gone too, ARM resumes the provider session (A10)
- [ ] M5b — **Session transcript** (the log view, a core feature): every turn of a session (user prompt, agent text and thinking, tool calls with arguments and results, messages) stored per session and read with one call (`GET /api/sessions/{id}/conversation`), plus counts (turns, messages, tool calls). Conversation content lives only here and in its events, never on `Session` or in lifecycle events (the `summary` field went on 2026-09-26); the `claude` provider already keeps each session's raw output (`output.jsonl`) to build from; the fake provider scripts turns so it's built LLM-free. Clients: `arm session conversation <id>` and the web UI's session detail ([`UI-REFERENCE.md`](UI-REFERENCE.md)). Following a running session live can come with M6: clients subscribe to the events and fetch the snapshot in parallel, buffer events until the snapshot arrives, then apply the ones it doesn't already contain (dedupe by a per-session sequence number that entries and events both carry, e.g. `seq` vs the snapshot's `lastSeq`). The web UI's session list gets the same treatment (today it can lose or overwrite an event while its first search loads)
