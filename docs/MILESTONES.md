...
- [ ] M9 — **Claude provider**, moved ahead of M5a (2026-09-26): its process model decides what gentle restart has to do. Investigate the integration first (ACP adapter per A10, Agent SDK, or the CLI's `stream-json` directly); then real Claude Code on the subscription login, health, record/replay LLM stub
