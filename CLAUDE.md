...
dotnet test --project Olve.AgentRuntimeManager.ApiTests     # API tests only (ARM_API_BASE_URL=… targets a running server)
dotnet run --project Olve.AgentRuntimeManager               # Run locally
```

From the repo root: `npx mise run ci` (the pipeline gate), `npx mise run codegen:test` (emitter
snapshots + the generator's conformance suite), `npx mise run api:image` (API tests against the
Docker image; needs Docker).
